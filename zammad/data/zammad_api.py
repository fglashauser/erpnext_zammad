...
class ZammadAPI:
    ...
    @property
    def ticket_time_accounting(self) -> "TicketTimeAccounting":
        """Return a `TicketTimeAccounting` instance"""
        return TicketTimeAccounting(connection=self)
...
class TicketTimeAccounting(Resource):
    path_attribute = "tickets"

    def get_by_ticket_id(self, ticket_id):
        """Returns all the time accountings associated with the ticket id

        :param ticket_id: Ticket id
        """
        response = self._connection.session.get(
            f"{self.url}/{ticket_id}/time_accountings"
        )
        return self._raise_or_return_json(response)
